...
void *central_command_server_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (rcvid <= 0) continue;  /* pulses (e.g. client disconnects) aren't used here */

        net_reply_t reply;
        memset(&reply, 0, sizeof(reply));

        if (cmd.type == NET_OVERRIDE_COMMAND) {
            /* Use the published safety view; step and countdown belong to
             * Phase_Controller_Task. The wait covers the rest of the whole
             * railway or pedestrian sequence plus 1s, so the CC's resend
             * arrives after it has ended. */
            ...
            if (view.rail_active) {
                ...
                snprintf(reply.reason, sizeof(reply.reason), "Railway protection active, wait until the crossing clears");
            } else if (view.ped_crossing) {
                reply.result = NET_RESULT_WAIT;
                reply.wait_seconds = seconds_until(view.busy_until_ms) + 1;
                snprintf(reply.reason, sizeof(reply.reason),
                         "Pedestrian crossing in progress, wait %d s", reply.wait_seconds);
            } else {
...
void apply_pending_cc_commands(lc_context_t *ctx)
{
    ...
    if (has_mode) {
        printf("[I%d][Phase_Controller_Task] MODE_SWITCH applied: mode=%s "
               "(until the next time-of-day period change)\n", ctx->id, lc_mode_name(new_mode));
        fflush(stdout);
        lc_set_mode(ctx, new_mode);
        notify_status(ctx, 0, 0, 0);   /* so the CC sees the new mode now */
    }

    if (!has_override) return;

    /* Check again: railway protection or a crossing may have started
     * since the server accepted the command, and both take priority. */
    ...
    case OVR_DIGNITARY_PATH:
        ...
        /* Hold in OVERRIDE_HOLD so the normal cycle and the sensor-driven
         * early end don't touch the forced green. */
        lc_enter_step_seconds(ctx, STEP_OVERRIDE_HOLD, OVERRIDE_HOLD_S);
        break;
